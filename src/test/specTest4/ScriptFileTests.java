...
public class ScriptFileTests {
    ...
    public static void calculateTestFiles() {
        ...
        try {
            ...
            for (File f : myDirectory.listFiles
            		(ScriptFileTests.testFileFilter)) {
     ...
     * ScriptFileTests constructor by the Parameterized test runner. Since 
     * that runner's constructor has one parameter, the array only has one 
     * element.
     */
    @Parameters
    public static List<Object[]> getTestFiles() {
        ...
        if (ScriptFileTests.testScriptFiles == null)
            throw new IllegalStateException("Did not initialise any files "
            		+ "to test");

        //we have to wrap testScriptFiles here so Parameterized.class receives
        // a list of arg array.
     ...
     *
     * @return List of labels to be used as names for each of the 
     * parameterized tests. These names are the same as the script file used 
     * to run the test.
     */
    @Labels
    public static List<String> getTestLabels() {
        if (ScriptFileTests.testScriptNames == null)
            throw new IllegalStateException("Must initialize list of test "
            		+ "names before creating tests.");
     ...
     * It creates a script file test instance, representing one script file 
     * to be tested.
     */
    ...
    private String runScriptFile() throws IOException {
        ...
        TestDriver4 td = new TestDriver4(testScriptFile, actual);
        td.runTests();
    ...
    /**
     * @param newSuffix
     * @return a File with the same name as testScriptFile, except that the 
     * test suffix is replaced by the given suffix
     */
    private File fileWithSuffix(String newSuffix) {
        ...
        String baseName = driverName.substring(0, driverName.length() 
        		- "test".length());
