...
/**
 * Runs the specification test files.
 *
...
public class ScriptFileTests {
   ...
   * ScriptFileTests constructor by the Parameterized test runner. Since that
   * runner's constructor has one parameter, the array only has one element.
   */
  @Parameters
  public static List<Object[]> getTestFiles() {
    ...
    // we have to wrap testScriptFiles here so Parameterized.class receives a 
    // list of arg array.
   ...
   * tests. These names are the same as the script file used to run the test.
   */
  @Labels
  public static List<String> getTestLabels() {
    if (ScriptFileTests.testScriptNames == null) {
      throw new IllegalStateException("Must initialize list of test names "
   ...
   * It creates a script file test instance, representing one script file to be
   * tested.
   */
  ...
  private String runScriptFile() throws IOException {
    ...
    TestDriver3 td = new TestDriver3(r, w);
    td.runTests();
